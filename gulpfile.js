...
gulp.task(
    ...
    gulp.series(function() {
        ...
        browserSync.init({
            proxy: {
                target: 'http://localhost:80/', // can be [virtual host, sub-directory, localhost with port]
    ...
    gulp.parallel([
        gulp.series([
            ...
            function runningWatch() {
                gulp.watch('./resources/sass/**/*', gulp.parallel('styles'));
...
gulp.task(
    ...
    gulp.series(
        ...
        function buildGULPHTML() {
            return gulp
                .src(['assets/views/**/*.pug', '!assets/views/{layouts,layouts/**}', '!assets/views/{includes,includes/**}'])
                .pipe(pug())
                .pipe(gulp.dest('./temp'));
        },
        /* =================== */
    ...
    gulp.parallel([
        gulp.series([
            ...
            function runningWatch() {
                ...
                gulp.watch('./resources/sass/app.scss', gulp.parallel('styles'));
...
gulp.task('static-build', gulp.series([gulp.series(['views', 'cleanTemp']), gulp.parallel(['styles', 'webpack:prod'])]));
